...
2. Read relevant tests to understand existing behavior
3. Check `git log --oneline -10` for recent changes in the area
...
3. **Save the plan** — write it to `working/plans/` (see Plan Storage below)
4. **Present to user** — explain the plan and wait for approval
5. **Only after approval** — exit plan mode
6. **Immediately save initial session log** — capture the goal, plan summary, and key context while it's fresh (see Session Logging below)
7. **Implement via orchestrator** — the orchestrator protocol takes over (see `orchestrator.md`): implement → verify → review → fix → score → present results
...
5. Check `git diff` for any uncommitted work
